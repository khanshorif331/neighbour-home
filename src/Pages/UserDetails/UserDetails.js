...
const UserDetails = () => {
     ...
     const { id } = useParams()
     const { isLoading, error, data: user, refetch } = useQuery(['singleUser'], () =>
               ...
               <div class={`${darkMode ? "bg-gray-700" : "bg-white"}   w-11/12 mx-auto shadow overflow-hidden sm:rounded-lg`}>
                    ...
                    <div class="border-t text-left border-gray-200">
                         <dl>
                              ...
                              <div class={`${darkMode ? "bg-gray-500" : "bg-gray-50"}  px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6`}>
                                   <dt class={`text-sm font-medium ${darkMode ? "text-gray-200" : "text-gray-500"} `}>Action</dt>
                                   <dd class={`${darkMode ? "text-white" : "text-gray-900"}  mt-1 text-sm  sm:mt-0 sm:col-span-2`}>
                                        {user?.role === 'admin' ?

                                             <button onClick={deleteUserAdmin} class='btn btn-xs btn-warning'>
                                                  Delete Admin
                                             </button>
                                             :
