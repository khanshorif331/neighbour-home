		...
		fetch(url, {
			...
			.then(result => {
				if (result.success) {
					...
					axios
						.post(
							`https://neighbour-home-backend.onrender.com/sellPost`,
							postInfo
						)
						.then(data => {
							console.log(data)
							if (data.status === 200) {
								toast.success(
									`${postInfo.propertyName} successfully Posted`
								)
					...
					>
						...
						<div className="text-right">
							<input
								...
								className="hover:bg-primary text-center transition hover:text-white rounded-full cursor-pointer text-primary border-2 border-primary w-36  text-sm sm:text-base  py-1.5 "
								type="submit"
								value="Submit"
							/>
						</div>
