...
const CheckoutForm = ({ property }) => {
	...
	const handleSubmit = async event => {
		...
		if (intentError) {
			...
			axios
				.post(
					`https://neighbour-home-backend.onrender.com/order`,
					orderInfo
				)
				.then(data => {
					...
					toast.success(
						`${property.propertyName} Succesfully Placed Order!`
					)
				})
			...
			axios
				.post(
					`https://neighbour-home-backend.onrender.com/notification`,
					notification
				)
				.then(data => {
					// console.log(data.data);
		...
		<div className="flex items-center border rounded w-10/12 sm:w-8/12 px-10 py-7 mx-auto sm:mx-0">
			...
			>
				...
				<input
					...
					name="buyer_message"
					type="text"
					value={`We have just received your payment for '${property.propertyName}' order. Thank you`}
				/>
				<input
					className="hidden"
					name="buyer_name"
